...
// address.ToConfidential, address.ToOutputScript, address.IsConfidential)
// hardcode the Liquid network HRPs ("ex"/"lq", "ert"/"el", "tex"/"tlq") via
// NetworkForAddress / DecodeType and therefore reject every Sequentia address
// (HRPs "bc"/"sqb", "bcrt", ...). The helpers below re-implement the same
// behaviour using the explicit-HRP low-level primitives (FromBech32/ToBech32/
// FromBlech32/ToBlech32 + base58) against an explicitly supplied Sequentia
// network. The go-elements v0.4.5 low-level address API used here is identical
// to the one the wallet's seqnet package targets, so this is a direct port.
